...
> 用户输入从 Composer 到 queue / steer / interrupt 的产品语义，本身就是 Adapter 行为契约的一部分；`user-input-lifecycle.md` 是该主题的专项设计与状态跟踪入口。对偶地，harness 输出从 wire 归一到终态收口（含丢事件自愈、静默悬挂对账）见 `harness-output-lifecycle.md`。本文继续给出完整交互面的总体分层、结构契约和 harness 映射，三者不是互斥边界。

本文解决两个问题：

1. chat-tui 如何覆盖 coding agent 常见的输入、输出与用户确认，同时继续保持纯 UI 组件边界；
2. baton Adapter 如何把 Claude Code、Codex 及未来 harness 归一成稳定语义，并以 ACP v2 作为主要词汇来源。

方案关注契约和演进顺序，不要求第一期实现所有 harness 能力。
...
| 工作控制 | 新 turn、steer、follow-up、interrupt、切模型/模式 | Controller；adapter 暴露可选能力 |
| agent 产出 | message、thought、tool、diff、plan、usage、error、notice | adapter 归一；baton 持久化；chat-tui 投影 |
| agent 请求用户 | permission、structured question、startup trust、elicitation | baton 分别建模；chat-tui 分别展示和回传 |

三层对象不能穿透：

```text
harness wire ── Adapter ──> baton semantic events ── projection ──> chat-tui view
harness wire <─ Adapter <── baton actions         <─ intents    <── chat-tui input
```

- chat-tui 不知道 HarnessSession、turn、ACP 或 Claude/Codex wire 消息。
- baton core 不依赖 chat-tui 的展示类型。
- Adapter 不返回 chat-tui block，也不把 harness DTO 直接写进 core state；原消息只放 envelope `raw`。

### 1.1 方案选择

| 方案 | 优点 | 问题 | 结论 |
|---|---|---|---|
| chat-tui 直接理解 Claude/Codex | 能快速复刻原生体验 | harness 分支进入组件库；无法独立复用 | 否决 |
| chat-tui 与 baton 全面采用 ACP DTO | 少设计一层类型 | ACP 是 wire protocol，且不覆盖 steer/process/subagent 等全部原生能力 | 否决 |
| UI view/intent + baton semantic event/action + harness adapter | 各层依赖稳定、支持本地/远端同构、可按能力渐进扩展 | 需要维护明确 projection | 采用 |

选中第三种方案，因为 projection 的显式成本小于 harness DTO 穿透后在 UI、存储、恢复和测试里形成的长期耦合。

## 2. 参考实现的可复用经验

### 2.1 pi-tui / pi-agent

- pi-tui 是低层组件系统：component、focus、overlay、editor、select/settings、markdown、image 各自独立。它不试图定义 agent event。这印证 chat-tui 应组合稳定 UI primitive，而不是复制 harness schema。
...
- AgentMessage 与 LLM Message 分离，通过转换层过滤 UI-only message。对应到 baton，就是统一历史不能等同于任一 harness 的 model-visible history。
...
- harness runtime 先归一成共同 LLM event，再交给 session processor；request lowering 与 transport/execution 分离。Adapter 同理应把 wire 翻译集中在 harness 目录，不把 harness 分支泄漏到 controller。
...
- 当前是否有 permission、question、hook trust 或 elicitation 需要回答；
- 哪些 command / config option 可选。

它不判断 harness 是否支持 steer，不维护 turn 队列，不执行 command，也不持久化 Interaction。
...
- baton 自有命令与 harness command 共用补全 UI，但 `id/owner` 不同；
- harness command 列表可随 session 状态动态替换，对齐 ACP `available_commands_update`；
...
Harness 或 Plugin 需要外部参与者给出结果后才能继续时，统一打开一个 **Interaction**。它是与 Input 正交的持久对象，**不限于权限**：

- `interactionId` 是 Baton 签发的稳定身份；
- `requester` 表示谁在等待结果，当前可为 HarnessTarget、PluginInstance 或 Baton；
- `kind ∈ {permission, question, hook_trust, ...}`，每个 kind 保持自己的 typed payload 与 resolution，不合成万能字段；
- 生命周期只有 `opened → resolved`。取消、超时、turn 收口和 crash recovery 都是明确的 resolution，不删除、不悬挂。

Adapter 只把 kind-specific draft 交给 Controller 并 `await` 结果。Controller 补
`interactionId + requester`、先 append `interaction.opened`，用户或策略作答后再 append
`interaction.resolved`，最后唤醒等待方。这样 identity、持久化、UI pending 投影和取消收口只有
一个 owner。

Interaction resolution 与 Input（prompt / steer）同为“用户 → 会话信号”，但状态机不同：
Input 驱动或注入 turn；resolution 必须引用已打开的 `interactionId` 并就地解阻，不进 queue。
`Event.source` 回答谁报告生命周期事实，`Interaction.requester` 回答谁在等待，两者不能混用。

`ApprovalReview` 不强行塞入 Interaction：auto-review 路由下 Harness 没有向 Baton 打开交互，
...
- 必须选择明确结果；interrupt/带外解决时按 interaction id 关闭；
...
- question Interaction 可带 toolCallId 与 auto-resolution metadata。
...
chat-tui 可以复用 select、editor、settings 等 primitive，但各 kind contract 不互相复用。OpenCode 的独立 Question/Permission service 与 Codex 的独立 overlay 都证明这种分离更稳。
...
harness 名猜测格式。工具输出、命令、diff 与 plan 继续走各自的展示 primitive。
...
`busy` 由 `runState.state === "running"` 派生，不再作为独立真相源。存在 pending Interaction 时，projection 必须产出 `requires_action`；反向不强制成立，因为登录、外部设备确认等 harness action 可能还没有对应的结构化 Interaction。此时 UI 必须展示通用 action-required notice，不能留下没有卡片的空白状态。

### 3.7 Overlay 与 pending input

- Controller 持有 Interaction lifecycle 真相源，chat-tui 只显示当前 pending Interaction；
- Interaction 以稳定 id 关闭，支持用户作答、用户 cancel、requester 带外解决、turn cancel、timeout 五种终态；timeout 的计时和终态决策由 Controller 持有，chat-tui 只展示 deadline/countdown，adapter 只负责把最终 resolution 映射回 harness；
- approval/question/elicitation 优先于 command/config picker；
- pending input 分别展示 requested delivery 与 effective delivery，例如 steer 被 harness 拒绝后降级为 follow-up，不能仍标记为 steer；
...
controller 出队（driven turn）
  → user_message upsert + state_update(running)   // controller 落盘：用户输入是 BatonSession 的事实，
...
Controller 以 state event 驱动 active/idle，不以 Promise 生命周期推断。cancel 的确认同样是最终 `idle/cancelled`，发送 cancel 后仍接受之前已在路上的 update。

生命周期必须满足“每个 baton turn 只发生一次逻辑终结”，而不是假设物理事件只会到达一次：reconnect、replay 和 transport race 都可能产生重复终态。Adapter 在正常 idle、wire fatal error、子进程退出和 transport close 上都必须报告或合成终态；controller 按 baton turn id 幂等 finalize，并忽略已完成 turn 的重复/迟到终态。

finalize 是统一的有序路径：持久化终态 → 生成一次 `_baton_turn_summary` → 更新同步元数据 → 释放该 turn 的等待者 → 推进输入队列。Adapter 丢失时 controller 只在确定 transport 已关闭，或 cancel grace period 到期后合成 terminal error；不设置任意的全局 turn 时长 watchdog，因为合法的长任务不应被误杀。没有 active baton turn 的后台事件也不能结束队首之外的工作。
...
- approval/sandbox 虽可展示为 config，但 adapter 必须保留 harness 安全语义和约束，不把任意 key/value 直接写入 wire；
- 现有 `/model`、`/effort` 分别成为 category=model / thought_level 的快捷入口，而不是独立状态真相源。

### 4.7 Interaction 生命周期

事件模型只使用同一对 lifecycle Event：
...
`opened.payload` 是带 `interactionId + requester` 的 typed Interaction；
`resolved.payload` 是 `interactionId + resolution`。permission、question、hook_trust 等都是
`kind`，不会各自复制一对事件名和 pending Map。

Adapter 通过宿主注入的 `InteractionHandler` 提交 draft：

```ts
type InteractionHandler = (
  interaction: InteractionDraft,
  context?: { turnId?: string; raw?: unknown },
) => Promise<InteractionResolution>;
```

- Controller 是唯一 lifecycle owner：签发 id、登记 waiter、append opened/resolved；
- Adapter 是 Harness wire owner：把 draft 和最终 resolution 映射到上游协议；
- Store/reducer 只维护一张 `interactions` 投影，`resolution` 缺省即 pending；
- UI 经 `controller.resolveInteraction(interactionId, resolution)` 作答；未知、过期或 kind
  不匹配的 resolution 返回 false，绝不按 allow 处理。

**cancel-cascade（已实现）**：turn 收口（尤其被 Esc 打断）时，Controller 将该 turn 仍挂起的
Interaction 统一 resolve 为 `{kind:"cancelled", reason:"turn"}`，先落
`interaction.resolved` 再解开 Adapter await；crash recovery 使用
`reason:"recovery"`。因此 live cancel 与 recovery 共享同一套“pending → resolved”语义，
不会残留 waiter 或 `requires_action`。

设计取舍（对照参考实现）：**interrupt 是 out-of-band 控制信号，不进 queue**——因为它要打断的正是 queue 里排在最前、且可能正阻塞在 pending Interaction 上的那个 turn，排队会死锁。三家印证同一形态:codex 内核把 interrupt/approval-response 都做成 `Op`（一条提交通道、turn 在独立 task 上跑，故 interrupt 能并发送达）；opencode 用 `Fiber.interrupt` + `ensuring(pending.delete)`；pi 用 `AbortSignal`。codex 还有一处精妙顺序——**先 abort task，再 clear pending waiters**，避免取消以 model 可见的 tool rejection 抢在 turn 中断之前冒出；baton 的 `finalize` 天然在 `adapter.cancel` 之后，顺序一致。
...
| 事件 | 原因 |
|---|---|
| `available_commands_update` | harness command 动态变化 |
...
当前 baton 的 `usage_update` 是 input/output/cache/reasoning token **增量**，与 ACP v2 `usage_update` 的 context used/size/cost **快照**同名不同义。v3 继续保留这一区分：`usage_update` 表示 delta，`context_usage_update` 表示快照，并由 adapter mapping 明确后者对应 ACP v2 `usage_update`。这是当前协议语义，不是对 v2 envelope 的兼容承诺。
...
1. ID owner 明确：Event/Interaction/BatonSession/turn 用 Baton ID；harness message/tool 原生 ID 保留映射，不能临时重造导致 upsert 断裂。
2. prompt acceptance 与 turn completion 分离；崩溃恢复能判断“已接受未完成”。
3. interaction.opened/resolved 可重放；reducer 不重新触发 harness side effect。
...
- controller 完全由 state event 驱动 busy/idle；
...
- permission / question / hook trust 已收敛为 Controller 持有的 Interaction opened/resolved 生命周期；
- Claude AskUserQuestion 与 Codex requestUserInput 通过 `kind:question` + chat-tui overlay 闭环；
- harness user dialog 尚未接入；
...
4. permission/question/hook_trust/elicitation：interaction.opened → requires_action → resolution/cancel/带外 resolved → running；
...
12. replay：与 live reduce 得到相同 view state，不重复触发 side effect；`usage_update` delta 累计结果稳定。
...
- permission/question/hook_trust/elicitation 的 focus、cancel、secret、multi-select、timeout；
...
- 不追求第一版完整复刻任一 harness 原生 TUI；
- 不为每个 harness event 新建 chat-tui block；
- 不在真实消费方出现前冻结 chat-tui 的公开类型；
- 不把 permission、question、hook_trust、elicitation 合成一个字段任意的万能 payload；它们只共享 Interaction identity/requester/lifecycle；
