...
Project
└── BatonSession
      ├── PluginInstance / PluginResource / Proposal
      ├── Turn
      ├── HarnessLaunchSnapshot ──▶ HarnessTarget
      └── HarnessSession
```

Project 目录只组织 BatonSession；Plugin 运行数据跟随 `batonSessionId`，同一 Project 下的
不同 Session 相互隔离。关键是不要继续让 cwd、Harness 名称或“当前值”同时承担以上三种身份。
恢复历史时使用当时的 HarnessLaunchSnapshot 解释执行事实；再次执行时仍要检查当前权限和
可用性，旧快照不是永久授权。
...
type EventEnvelope = {
  v: 3;
  eventId: string;
  scope: { type: "session"; batonSessionId: string };
  ts: string;
  seq: number;
  source: EventSource;
...
2. 从当前 BatonSession 的 PluginInstance、Resource 和 Proposal 恢复领域 loop；
