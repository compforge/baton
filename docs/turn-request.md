...
Allow 后先固化 Target、预签发 `messageId/turnId` 并持久化 scheduled fact，再把 Input 放入普通
全局队列。TurnRequest 的语义就是请求一个 new Turn：Harness busy 时只排队，不能 steer 到当前
Turn；调用 Harness 是 Input 准入后的下游行为，不是 Request 自己执行 Harness Work。
...
- `_baton_turn_request_scheduled`：记录 Input 的 messageId、turnId 和 HarnessTarget；
...
缺省取消最近一个可取消的 TurnRequest。queued Request 被移出 InputQueue 并进入 `cancelled`；已经
admitted 的 Request 走普通 interrupt，最终进入 `completed`，成败由 TurnSummary 表达。
...
- 已 scheduled 但没有 `user_message`：用原 identity 重建 Input；
...
Input source 全链路、busy 时不 steer、user recall 不撤 Plugin Input、queued/running 取消分流、崩溃
窗口恢复不重复执行、`uncertain` 不重投、timeline 的 Plugin 来源标注，以及 TurnSummary 落盘后原
Resource 获得准确结果。
