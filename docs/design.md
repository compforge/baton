...
| 5 | 会话数据存哪 | `~/.baton/projects/<project key>/sessions/<id>/`：事件、诊断、元数据与 session-scoped Plugin runtime 共址；原生会话只用于加速恢复 | 已定 |
...
Composer          输入框（现在时）
  ...
  └ Interaction Dock  补全 / 选择 / 审批 / 问题，锚定输入框
[Toast]           短寿命操作回执 / 错误提示（有内容才渲染；不替换 Footer）
...
- **Toast 与 Footer 寿命不同，不互相替换**：Toast 只承载当前操作的短寿命回执或错误（如 steer 降级、命令结果、过期请求），状态变化后即可消失；Footer 是用户随时可查的常驻会话状态，即使 Toast 出现也必须保留。需要跨操作回看的 warning / error 不放 Toast，应作为 notice 进入 Transcript。
