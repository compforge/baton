...
function sessionWithTurns(turnTexts: Array<{ user: string; agent: string }>): SessionHandle {
  ...
  turnTexts.forEach((t, i) => {
    const turnId = `t_${i}`;
    h.ledger.append({ source: { type: "baton" }, kind: "user_message", harness: "codex", turnId, payload: { messageId: `${turnId}_u`, content: [{ type: "text", text: t.user }] } });
    h.ledger.append({ source: { type: "baton" }, kind: "agent_message", harness: "codex", turnId, payload: { messageId: `${turnId}_a`, content: [{ type: "text", text: t.agent }] } });
    h.ledger.append({ source: { type: "baton" }, kind: "state_update", harness: "codex", turnId, payload: { state: "idle", stopReason: "end_turn" } });
