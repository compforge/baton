...
describe("shell command read effects", () => {
  ...
  test("accepts the session's structured read compositions", () => {
    expectCommands([
      ...
      "ls -d ~/.claude ~/.codex ~/.deepseek* ~/.dsh* ~/.baton 2>/dev/null",
      "ls foo?.ts [ab].ts {src,tests}",
      "cat devloop/.claude-plugin/*.json",
      "head -80 a.ts | grep x",
      "grep -rn \"model\\|\\\"quoted\\\"\" lib | sort | uniq -c | sort -rn",
      "which dsh",
  ...
  test("rejects mutations and effectful query options", () => {
    expectCommands([
      ...
      "uniq --skip-fields 1 input.txt output.txt",
