...
- **Plugin 通过 Resource 的统一 reconcile 路径工作**：Builtin Resource 是 Event Ledger 的只读 replay/live 投影；PluginResource 保存 `spec/status`。Resource 变化、`requeueAfter` 和 Resource Contribution 的固定 cron schedule 都只 enqueue 同一 keyed reconcile，schedule 不直接执行回调或修改 status；Plugin 不必先创建 Resource、更不必是 loop。
