...
- **Board 是带归属的派生读模型**：Plugin 通过 Controller 的 `print(resource)` 决定每份 Resource 是否展示，Baton 补齐 owner 与 Resource 身份；业务 phase 只属于 Plugin status，Baton 不定义通用 lifecycle。无数据时 UI 完全隐藏，Board 不成为 Resource 或外部系统之外的新真相源。整体边界见 `docs/loop-engineering.md` §6。
